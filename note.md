...
特征工程 (Feature Engineering) 是指从原始数据中选择、变换、构造特征，以提高模型的表现能力。

它通常包括：
1. **特征选择**：从大量特征中选择最相关、最有用的特征。
2. **特征变换**：对特征进行数学变换（如标准化、对数变换、编码）。
3. **特征构造**：从现有特征中创建新的特征，如组合特征、聚合特征等。

# ACF & PACF


# XGboost
extreme gradient boosting (XGBoost) 是一种基于决策树的集成学习算法，它在传统的梯度提升算法基础上进行了改进和优化，以提高模型的性能和效率。

### 特点：
1. Boosting 思想：多个弱模型（决策树）逐步迭代，每次纠正上一次的错误。
2. 正则化：防止过拟合，能控制模型复杂度。
3. 支持并行化：训练速度很快。
4. 灵活性：可以做回归、分类、排序，也能处理缺失值。

### 在时间序列里的应用
在时间序列预测里，XGBoost 不能直接“看时间”，通常要先做 特征工程，比如：

- 滞后特征（用你之前的 make_lags）
- 滑动平均、滚动窗口统计
- 时间特征（年/月/周/日/节假日等）

# 正则化（Regularization）
正则化就是 在模型训练时给损失函数加一项惩罚项，目的是：
